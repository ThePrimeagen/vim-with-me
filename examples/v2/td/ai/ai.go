...
func NewGemini(system string, ctx context.Context) *Gemini {
    client := anthropic.NewClient(os.Getenv("ANTHROPIC_API_KEY"))
    return &Gemini{
        ctx: ctx,
        client: client,
...
func (o *Gemini) chat(chat string, ctx context.Context) (string, error) {
...
func (s *Gemini) Name() string {
    return "anthropic"
}

func (s *Gemini) ReadWithTimeout(p string, t time.Duration) (string, error) {
    ctx, cancel := context.WithCancel(s.ctx)
    go func() {
        <-time.NewTimer(t).C
        cancel()
